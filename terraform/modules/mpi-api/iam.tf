...
resource "aws_iam_role" "mpi_write_lambda_role" {
  name = "mpi_write_lambda_role-${var.region_suffix}"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy" "mpi_write_lambda_policy" {
  name = "mpi_write_lambda_policy-${var.region_suffix}"
  role = aws_iam_role.mpi_write_lambda_role.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
      ...
      },
      {
        ...
        Effect   = "Allow"
        Resource = aws_cloudwatch_log_group.mpi_write_lambda_log_group.arn
      }
    ]
  })
}