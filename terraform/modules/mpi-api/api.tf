...
# write lambda api:
resource "aws_apigatewayv2_integration" "mpi_api_write_lambda_integration" {
  api_id             = aws_apigatewayv2_api.mpi-api.id
  integration_type   = "AWS_PROXY"
  integration_uri    = aws_lambda_function.mpi_write_lambda.arn
  integration_method = "POST"
}

resource "aws_apigatewayv2_route" "mpi_api_write_route" {
  api_id    = aws_apigatewayv2_api.mpi-api.id
  route_key = "POST /mpi"
  target    = "integrations/${aws_apigatewayv2_integration.mpi_api_write_lambda_integration.id}"
}

resource "aws_lambda_permission" "mpi_api_write_lambda_permission" {
  statement_id  = "AllowAPIGatewayInvokeWrite"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.mpi_write_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.mpi-api.execution_arn}/*/*"
}